...
ff_read(struct img *img)
{
	...
	for (off = 0, i = 0; i < img->height; ++i) {
		if (fread(row, 1, (size_t)row_len, img->fp) != (size_t)row_len) {
			free(row);
			die("%s: unexpected EOF or row-skew at %d\n", argv0, i);
		}
...
loadimgs(void)
{
	...
	for(i=0 ; i<nimgs ; ++i){
		if (ff_open(&imgs[i]))
			die("%s: %s: can't open image (invalid format?)\n", argv0, imgs[i].filename);
		if (ff_read(&imgs[i]))
			die("%s: can't read image\n", argv0);
	}
}
...
ximage(unsigned int newwidth, unsigned int newheight)
{
	...
	if (depth >= 24) {
		...
		die("%s: this program does not yet support display depths < 24\n", argv0);
	}
}
